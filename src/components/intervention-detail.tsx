  ...
  return (
    <div className="space-y-5 text-[#172033]">
      ...
      <section className={`rounded-2xl border p-4 sm:p-5 ${current.status === "completed" || invoice ? "border-violet-200 bg-violet-50/40" : "border-zinc-200 bg-zinc-50"}`}>
        ...
        {current.status !== "completed" && !invoice ? <p className="mt-5 rounded-xl border border-dashed border-zinc-300 p-5 text-center text-xs text-zinc-500">Cette zone sera disponible dès que la prestation sera terminée.</p> : !invoice ? <div className="mt-5 grid gap-4"><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><Field label="Facture Henrri du client"><Select value={invoiceChoice} onChange={(event) => setInvoiceChoice(event.target.value)}><option value="">Sélectionner une facture importée…</option>{eligibleInvoices.map((item) => <option key={item.id} value={item.id}>{item.number} · {formatMoney(item.totalIncludingTax)} · {formatDate(item.issuedAt)}</option>)}</Select></Field><Button className="self-end" disabled={!invoiceChoice} onClick={linkInvoice}><ReceiptText className="size-4" /> Associer</Button></div><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-ink-900 p-4"><p className="text-xs text-zinc-600">La facture n’est pas encore dans ADetailing ? Importez son PDF puis revenez l’associer ici.</p><Link href="/documents?tab=imports"><Button size="sm" variant="secondary"><FileUp className="size-3.5" /> Importer le PDF</Button></Link></div></div> : <div className="mt-5 grid gap-4"><div className="grid gap-3 rounded-2xl bg-ink-900 p-4 sm:grid-cols-4"><div><p className="text-[10px] font-bold text-zinc-500 uppercase">Facture</p><p className="mt-1 text-sm font-bold">{invoice.number}</p></div><div><p className="text-[10px] font-bold text-zinc-500 uppercase">Total TTC</p><p className="mt-1 text-sm font-bold">{formatMoney(invoice.totalIncludingTax)}</p></div><div><p className="text-[10px] font-bold text-zinc-500 uppercase">Encaissé</p><p className="mt-1 text-sm font-bold text-emerald-700">{formatMoney(workflow.paidAmount)}</p></div><div><p className="text-[10px] font-bold text-zinc-500 uppercase">Reste</p><p className="mt-1 text-sm font-bold text-brand-700">{formatMoney(workflow.outstanding)}</p></div><div className="sm:col-span-4 flex flex-wrap items-center justify-between gap-2 border-t border-zinc-100 pt-3"><StatusBadge status={paymentStatusForInvoice(invoice, data.payments)}>{paymentStatusLabels[paymentStatusForInvoice(invoice, data.payments)]}</StatusBadge><button className="text-[10px] font-semibold text-zinc-500 hover:text-red-600" onClick={() => { data.linkInvoiceToIntervention(current.id, undefined); setInvoiceChoice(""); toast.info("Facture dissociée"); }}>Changer de facture</button></div></div>{workflow.outstanding > 0 && <div className="grid gap-3 rounded-2xl border border-emerald-200 bg-ink-900 p-4 sm:grid-cols-[1fr_1fr_auto]"><Field label="Montant reçu (€)"><Input min="0.01" max={workflow.outstanding / 100} step="0.01" type="number" value={paymentEuros || workflow.outstanding / 100} onChange={(event) => setPaymentEuros(Number(event.target.value))} /></Field><Field label="Moyen de paiement"><Select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}><option>Carte</option><option>Virement</option><option>Espèces</option><option>Chèque</option></Select></Field><Button className="self-end" onClick={addPayment}><CircleDollarSign className="size-4" /> Encaisser</Button></div>}{data.payments.some((payment) => payment.invoiceId === invoice.id) && <div><p className="mb-2 text-xs font-bold">Historique des paiements</p><div className="grid gap-2">{data.payments.filter((payment) => payment.invoiceId === invoice.id).map((payment) => <div key={payment.id} className="flex items-center justify-between rounded-xl bg-ink-900 px-4 py-3 text-xs"><span>{formatDate(payment.paidAt)} · {payment.method}</span><strong>{formatMoney(payment.amount)}</strong></div>)}</div></div>}</div>}
      </section>
