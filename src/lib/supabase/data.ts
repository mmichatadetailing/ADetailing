...
export async function loadSupabaseAppData(supabase: SupabaseClient, user: User): Promise<BootstrapPayload> {
  ...
  const results = await Promise.all([
    ...
    supabase.from("services").select("*,service_categories(name),service_prices(amount_cents,vehicle_formats(name)),service_aliases(alias)").eq("organization_id", organizationId).is("archived_at", null).order("display_order"),
  ...
  const services: Service[] = serviceRows.map((row) => ({
    ...
    clientDescription: text(row, "client_description"), internalDescription: text(row, "internal_description"), prices: rows(row.service_prices).map((price) => ({ vehicleFormat: text(one(price.vehicle_formats), "name", "Tous formats"), amount: number(price, "amount_cents") })),
