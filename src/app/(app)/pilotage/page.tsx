...
import { collectedInterventionRevenue, conversionRate, grossMargin, hourlyMargin } from "@/lib/domain/calculations";
...
export default function PilotagePage() {
  ...
  const currentMonthPayments = data.payments.filter((payment) => payment.paidAt.slice(0, 7) === currentMonth);
  const revenue = collectedInterventionRevenue(data.interventions.filter((intervention) => intervention.status !== "cancelled"), currentMonthPayments);
  const completed = data.interventions.filter((item) => item.status === "completed" && item.startAt?.slice(0, 7) === currentMonth);
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[
        ["CA encaissé", formatMoney(revenue), `${Math.round(revenue / Math.max(objective?.revenueTarget ?? 1, 1) * 100)} % de l’objectif`], ["Marge brute", formatMoney(margin), "prestations terminées ce mois"], ["Marge horaire", `${formatMoney(avgHourly)}/h`, `objectif ${formatMoney(data.settings.hourlyMarginTarget)}/h`], ["Conversion", `${Math.round(conversionRate(data.leads) * 100)} %`, `objectif ${data.settings.conversionTargetBasisPoints / 100} %`],
      ...
      <Card><CardContent className="p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center"><span className="grid size-11 place-items-center rounded-2xl bg-brand-500/10 text-brand-400"><Target className="size-5" /></span><div className="flex-1"><div className="flex items-center justify-between"><div><p className="text-sm font-bold capitalize">Objectif de {currentMonthLabel}</p><p className="mt-1 text-xs text-zinc-600">{formatMoney(revenue)} réellement encaissé sur {formatMoney(objective?.revenueTarget ?? 0)}</p></div><p className="text-lg font-bold text-brand-300">{Math.round(revenue / Math.max(objective?.revenueTarget ?? 1, 1) * 100)} %</p></div><Progress value={revenue / Math.max(objective?.revenueTarget ?? 1, 1) * 100} className="mt-3" /></div></div></CardContent></Card>
