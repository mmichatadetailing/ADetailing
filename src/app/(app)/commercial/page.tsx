...
function LeadCard({ lead }: { lead: Lead }) {
  ...
  return (
    <article ref={setNodeRef} style={{ transform: transform ? `translate3d(${transform.x}px, ${transform.y}px, 0)` : undefined }} className={`rounded-2xl border border-white/[0.075] bg-ink-850 p-3.5 shadow-lg transition ${isDragging ? "z-50 opacity-70 shadow-2xl" : "hover:border-white/[0.13]"}`}>
      ...
      <div className="mt-4"><p className="truncate text-xs font-medium text-zinc-300">{lead.serviceLabel}</p><p className="mt-1 text-sm font-bold text-white">{formatMoney(lead.estimatedAmount)}</p></div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-white/[0.06] pt-3"><div className="flex items-center gap-2"><Avatar label={owner?.initials ?? "?"} color={owner?.color} size="sm" /><span className="text-[10px] text-zinc-600">{formatDate(lead.requestedAt)}</span></div><a className="focus-ring grid size-7 place-items-center rounded-lg text-zinc-600 hover:bg-white/5 hover:text-white" href={`tel:${lead.phone}`}><Phone className="size-3.5" /></a></div>
    </article>
  );
}
